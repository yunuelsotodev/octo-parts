...
- **Rules:** the rules of feature-arch v{{SOURCE_VERSION}} ({{RULES_APPLIED_COUNT}} applied)
...
<!-- Omit this section on overall PASS. Aggregate the reviewer's "missing for PASS"
     suggestions, dedupe, keep locations, order by the source skill's category priority:
     struct → import → bound → fquery → fcomp → fstate → test → name.
