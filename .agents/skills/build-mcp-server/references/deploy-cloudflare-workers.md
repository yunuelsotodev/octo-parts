...
**Prerequisite:** `wrangler deploy` requires a Cloudflare account. Run `npx wrangler login` first if you haven't authenticated.

**Teardown:** `npx wrangler delete my-mcp-server` removes the Worker and its Durable Objects.
