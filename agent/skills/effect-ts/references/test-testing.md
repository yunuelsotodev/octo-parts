...
title: "Testing"
impact: LOW
impactDescription: "Enables deterministic time-dependent tests — covers TestClock"
tags: test, testing, testclock, time
---
...
**Incorrect (real delays in tests):**

```ts
const result = await Effect.runPromise(
  Effect.delay(myEffect, "5 seconds") // Test waits 5 real seconds
)
```

**Correct (using TestClock to simulate time):**

```ts
const result = yield* TestClock.adjust("5 seconds").pipe(
  Effect.zipRight(myEffect) // Completes instantly in tests
)
```
